...
class TelegramAlertChannel(AlertChannel):
    ...
    def __init__(self, bot_token: str, chat_id: str | int) -> None:
        ...
        log.debug(
            "telegraAlertChannel initialised base_url=%s,chat_id=%s",
            self.base_url,
            self.chat_id,
        )
    ...
    def send(self, message: str) -> None:
        ...
        this block for up to the ``timeout`` define on the request(10 seconds).
        On any non 200 responce.
        we raise ``AlertSendError`` - the dispatcher catches this per channel and 
        records a "failed" status in the database.
        Parameters -- message:str (the raw text to deliver , 
        telgram will render markdown style.).
        """
        payload = {
        ...
        if not resp.ok:
            ...
            try:
                ...
                detail = resp.text
            raise AlertSendError(f"telegram api error {resp.status_code}:{detail}")
